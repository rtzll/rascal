...
func (l DockerLauncher) StartDetached(ctx context.Context, spec Spec) (handle ExecutionHandle, err error) {
	...
	envPairs := map[string]string{
		...
		"RASCAL_VALIDATION_ENABLED":                     strconv.FormatBool(spec.Validation.Enabled),
		"RASCAL_VALIDATION_CRITIQUE":                    strconv.FormatBool(spec.Validation.CritiqueEnabled),
		"RASCAL_VALIDATION_TEST_CRITIQUE":               strconv.FormatBool(spec.Validation.TestCritiqueEnable),
		"RASCAL_VALIDATION_BLOCK_ON_DETERMINISTIC_FAIL": strconv.FormatBool(spec.Validation.Gate.BlockOnDeterministicFailure),
		"RASCAL_VALIDATION_BLOCK_ON_CRITIQUE_BLOCKER":   strconv.FormatBool(spec.Validation.Gate.BlockOnCritiqueBlocker),
		"RASCAL_VALIDATION_BLOCK_ON_CRITIQUE_WARNING":   strconv.FormatBool(spec.Validation.Gate.BlockOnCritiqueWarning),
