...
func (l DockerLauncher) StartDetached(ctx context.Context, spec Spec) (handle ExecutionHandle, err error) {
	...
	envPairs := map[string]string{
		...
		"RASCAL_REVIEW_LOOP_ENABLED":            strconv.FormatBool(spec.ReviewLoopEnabled),
		"RASCAL_REVIEW_MAX_INITIAL_PASSES":      strconv.Itoa(spec.ReviewMaxInitialPasses),
		"RASCAL_REVIEW_MAX_FIX_PASSES":          strconv.Itoa(spec.ReviewMaxFixPasses),
		"RASCAL_REVIEW_MAX_VERIFICATION_PASSES": strconv.Itoa(spec.ReviewMaxVerificationPasses),
		"RASCAL_DETERMINISTIC_CHECK_COMMANDS":   strings.TrimSpace(spec.DeterministicCheckCommands),
